Public Class FormPublisher
    ...
    Private Sub addNewPublisher()
        objPublisher.PublisherId = LibraryDatabase.GenerateID("SELECT MAX(publisherId) FROM `publisher`;")
    ...
    Private Sub btnEditPublisher_Click(sender As Object, e As EventArgs) Handles btnEditPublisher.Click
        If btnEditPublisher.Text = "Save" Then
            If Not String.IsNullOrWhiteSpace(txtPublisher.Text) Then
                If dgvPublisher.Rows.Count > 0 Then
                    If LibraryDatabase.isExisting("SELECT COUNT(*) FROM publisher WHERE publisherName='" + txtPublisher.Text + "';") = False Then
                        addNewPublisher()
        ...
        ElseIf btnEditPublisher.Text = "Update" Then
            If Not String.IsNullOrWhiteSpace(txtPublisher.Text) Then
                If dgvPublisher.Rows.Count > 0 Then
                    If LibraryDatabase.isExisting("SELECT COUNT(*) FROM publisher WHERE publisherName='" + txtPublisher.Text + "' and publisherId!=" + dgvPublisher.SelectedRows(0).Cells(0).Value.ToString + ";") = False Then
